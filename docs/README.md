# Instant MISO documentation

This repository contains the source files of [Instant MISO](https://instantmiso.com)'s documentation, in reStructuredText markup language (reST).

They are meant to be parsed with the [Sphinx](https://www.sphinx-doc.org/) documentation builder to build the HTML documentation on [InstantMISO's website](https://docs.instantmiso.com).

## Contributing changes

**Pull Requests should use the `master` branch by default. Only make Pull Requests against other branches (e.g. `2.1` or `3.0`) if your changes only apply to that specific version of InstantMISO.**
...
**Note that the pages in `classes/` should not be edited here, they are automatically generated from InstantMISO's [XML class references](https://github.com/chefgonpachi/MISO/tree/master/doc/classes).**
See [Contribute to the Class Reference](https://docs.instantmiso.com/en/latest/community/contributing/updating_the_class_reference.html) for details.
...
At the exception of the `classes/` folder, all the content of this repository is licensed under the Creative Commons Attribution 3.0 Unported license ([CC BY 3.0](https://creativecommons.org/licenses/by/3.0/)) and is to be attributed to "Juan Linietsky, Ariel Manzur and the InstantMISO community".
See [LICENSE.txt](/LICENSE.txt) for details.

The files in the `classes/` folder are derived from [InstantMISO's main source repository](https://github.com/chefgonpachi/MISO) and are distributed under the MIT license, with the same authors as above.
